    ...
    <div className="sticky top-20 z-30 bg-black/95 backdrop-blur-xl border-b border-white/5">
      <div className="max-w-7xl mx-auto px-4 py-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            {genres.map(g => (
              <option key={g} value={g} className="bg-black">{g === 'All' ? t('allGenres') : g}</option>
            ))}
          </select>
          ...
          >
            {bpmRanges.map(b => (
              <option key={b} value={b} className="bg-black">{b === 'All' ? t('allBPM') : `${b} BPM`}</option>
            ))}
          </select>
          ...
          >
            {keys.map(k => (
              <option key={k} value={k} className="bg-black">{k === 'All' ? t('allKeys') : k}</option>
            ))}
          </select>
          ...
          >
            {moods.map(m => (
              <option key={m} value={m} className="bg-black">{m === 'All' ? t('allMoods') : m}</option>
            ))}
          </select>
